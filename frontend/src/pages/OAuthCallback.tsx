...
const OAuthCallback = () => {
      ...
      handleOAuthCallback(token)
        ...
        .catch((err) => {
          ...
          window.dispatchEvent(
            new CustomEvent('open-login-modal', { detail: { error: err?.message || 'OAuth login failed' } })
          );
        });
    } else {
  ...
  return (
    <Box minH="100vh" bg="gray.900" position="relative" overflowX="hidden" w="100%">
      <StarryBackground />
      <Box display="flex" alignItems="center" justifyContent="center" minH="100vh" position="relative" zIndex={1}>
        <Box
        ...
        >
          ...
          <VStack spacing={8} color="white">
            <Heading size="lg" bgGradient="linear(to-r, purple.300, blue.300)" bgClip="text" fontWeight="bold" style={{ marginTop: '5%' }}>
              Processing Authentication
            </Heading>
            <Spinner size="xl" thickness="4px" speed="0.65s" emptyColor="gray.600" color="purple.300" />
            <Text color="gray.300" fontSize="md">
              Please wait while we securely log you in...
            </Text>
            {error && (
              <Alert status="error" borderRadius="xl" bg="rgba(254, 202, 202, 0.1)" border="1px solid" borderColor="red.300" color="red.200">
                <AlertIcon />
                {error}
              </Alert>
            )}
